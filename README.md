...
| <a name="isVirtuallyCoached"></a> ([`schema:Event`](https://schema.org/Event)) <br/>  `beta:isVirtuallyCoached` | [`schema:Boolean`](https://schema.org/Boolean) | [#71](https://github.com/openactive/modelling-opportunity-data/issues/71) | A property that indicates whether the event is led by a virtual coach. Only relevant if a class `isCoached`. |
